...
simulation_BIC_penHubs = function(Theta, args,lambda, threshold, folder, save=FALSE){
  set.seed(args$seed)
  sim_data = sample_from_precision(args$n, args$p, Theta)
  X = sim_data$X
  ...
  #Fit GLASSO
  Theta_GL_Lik = BICweighted_glasso(mat = CoVR_X$Cov, rho = lambda, .W = w_icphd, p = args$p, n = args$n,
                                       threshold = 0, maxit = 200,
                                       penalize.diagonal = FALSE)
  W = w_icphd
  ...
  if(save){
    #Save results to analize
    folder_path = paste0(folder, '/penHubs_BIC')
...
simulation_Lik_penHubs = function(Theta, args,lambda, threshold, folder, save=FALSE){
  set.seed(args$seed)
  #simulate data
  sim_data = sample_from_precision(args$n, args$p, Theta)
  X_train = sim_data$X_train
  ...
  #Fit GLASSO
  Theta_GL_Lik = Lik_weighted_glasso(mat = CoVR_X$Cov,mat_val = CoVR_X_val$Cov, rho = lambda, .W = w_icphd, p = args$p, n = args$n,
                                       threshold = 0, maxit = 200,
                                       penalize.diagonal = FALSE)
  ...
  if(save){
    #Save results to analize
    folder_path = paste0(folder, '/penHubs_Lik')
